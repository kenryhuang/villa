...
The project uses Godot's `gl_compatibility` renderer, where `Decal` is unsupported. The ground therefore uses one `ArrayMesh` with a transparent `StandardMaterial3D`. It clips the rendered terrain's own 96×96 triangles to the production footprint, preserves their height planes, and lifts the result by 0.028 world units. Sharing the exact tessellation prevents both vertex-level and between-vertex terrain intersections. Because the art is drawn only on its own mesh, it cannot project onto buildings, products, actors, crops, or UI feedback and does not require a special terrain render layer.
...
- every ground vertex and densely sampled triangle interior stays above and aligned with the actual rendered terrain triangles;
