...
**Goal:** Replace every production-yard fence and its collision with one walkable, hand-painted, semi-transparent terrain-projected ground patch while preserving output-slot and save compatibility.

**Architecture:** Keep `BuildingProductionYard` and the `ProductionYard` node/API as the compatibility boundary. Internally, replace fence `Sprite3D` segments, construction atlases, transitions, and `StaticBody3D` perimeter collisions with a single `Decal`. The decal projects only onto the terrain mesh through a dedicated visual layer/cull mask; building bodies remain the only physical obstruction. Three 1024×1024 RGBA textures cover timber, masonry, and industrial production-yard families.

**Tech Stack:** Godot 4.7 GDScript, `Decal`, `MeshInstance3D.layers`, PNG RGBA assets, the existing headless GDScript test harness, and the built-in image generation tool.
...
- Configuring a 3×3 or 4×4 yard creates exactly one `Decal`, sized to the footprint plus at most 0.2 world units total overhang per axis.
- The decal uses the terrain-only cull mask.
...
- `set_construction_stage()` stores the requested stage but leaves the same ground decal and texture in place with no transition visuals.
- Preview state applies green/red tint; maintenance state does not tint the ground.
- `clear_immediately()` removes the decal and keeps compatibility query methods safe.

Add small recursive helpers for finding `Decal`, `Sprite3D`, and `StaticBody3D` nodes. Keep assertions focused on observable behavior, not private field names.
...
**Step 3: Add terrain projection-layer assertions**

Extend `tests/test_terrain_builder.gd` to build a real `TerrainBuilder`, find `TerrainMesh`, and assert that its visual layers include both the default render layer and `TerrainBuilder.PRODUCTION_GROUND_RENDER_LAYER`. Also assert that `TerrainBody` retains collision layer 1.
...
Expected: failures for missing `*_yard_ground.png`, missing decal/query behavior, old fence collisions, and the missing terrain render-layer constant.
...
### Task 3: Replace fence runtime with a terrain-only decal

**Files:**
- Modify: `scripts/buildings/building_production_yard.gd`
- Modify: `scripts/world/terrain_builder.gd`

**Step 1: Add a dedicated terrain visual layer**

In `TerrainBuilder`, define one shared bit, avoiding existing default layer semantics:

```gdscript
const PRODUCTION_GROUND_RENDER_LAYER := 1 << 7
```

After constructing `TerrainMesh`, set:

```gdscript
mesh_instance.layers = 1 | PRODUCTION_GROUND_RENDER_LAYER
```

This keeps normal terrain rendering while making it the only receiver for production-ground decals.
...
- `GROUND_DECAL_HEIGHT` and shallow projection depth sufficient for the terrain height variation beneath one footprint
- `GROUND_TEXTURES` pointing to the three new PNGs
- one `_ground_decal: Decal`
...
**Step 3: Build one terrain-projected ground decal**
...
4. Create one named `GroundDecal` if the texture is valid.
5. Assign `texture_albedo`, footprint-sized `size`, and `TerrainBuilder.PRODUCTION_GROUND_RENDER_LAYER` as `cull_mask`.
6. Position its projection volume so it reaches the local terrain without touching building geometry; the cull mask is the hard guarantee that buildings, products, actors, and crops cannot receive it.
...
- Add `get_ground_visual_count()`, `get_ground_size()`, and `get_ground_cull_mask()` for observable testing/debugging.
- `clear_immediately()` removes the decal immediately and clears output slots.

**Step 5: Apply preview tint only**

`get_visual_tint()` returns green/red for active preview and white otherwise. `set_maintenance_state()` retains the state for compatibility but does not modify the decal. Apply tint through `Decal.modulate`, retaining alpha in the texture itself.
...
- the terrain retains its standard layer plus the decal receiver layer;
