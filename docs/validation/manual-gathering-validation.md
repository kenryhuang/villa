...
| `tests/run_main_gameplay_integration_tests.gd` | PASS：1005 |
| `tests/run_resource_gathering_tests.gd` | PASS：170 |
| `tests/run_gathering_visual_tests.gd` | PASS：81 |
...
| 伐木动作与三帧 | 通过 | 通过 | 通过 | 种地方格阴影隐藏；斧柄末端为固定圆心，斧头头部从左向右做扇形往复并命中低处切口 |
