...
3. **A/D** 调整瞄准，**Shift+A/D** 精细瞄准（转速约为普通的 1/6），**Q/E** 调整人与球的站距（0.68–0.98 米）；**W/S 或滚轮** 上下调整触球点，操作条上的球形图示显示当前触球位置。击球下部配合大力度可挑起球，中心轻击则沿地面滚动。**1/2/3** 切换开球杆、挖起杆、推杆，并设置对应的默认触球点。果岭推荐推杆，沙坑推荐挖起杆。
4. **按住鼠标左键，向后拉，再向前推过击球点**。后拉距离决定最终力度，前推速度不再改变力度。HUD 显示实时百分比及挥杆轨迹，横移落在绿色容错区内直出；明显斜推才偏球，普通挥杆偏角上限约 3.4°、推球约 1.2°。推球采用更细的小力度曲线。后拉满格不会积累看不见的额外行程，提前松开仍取消。
...
贴地滚动的球只要路径经过可见洞口就落洞，不设速度上限。判定检查整个移动线段，覆盖洞口中心和内沿，避免快速滚动时一帧跨过洞口却漏判。空中越洞或从洞口外侧擦过不会被吸入。洞口同时切开地形网格与碰撞，杯壁和杯底采用立体几何。

球进入洞口后，先显示 0.4 秒的落杯过程，再结算并显示“进洞”。落杯期间不提前完成本洞，同一杆只结算一次。完成后仍需走到下一发球台按 E。[落洞实景](images/golf_3d_cup_drop.png)

越过场地外侧缓冲边界或进入水面，按本场规则额外罚 1 杆，将球放回本杆起点。当前球道均位于干燥陆地。虚线复用实际球物理，只展示前 1.2 秒滚动或首次落地前的飞行（最多 5 秒），包含坡度、重力和材质阻力；不预告最终停点或自动进洞，也不预判动态障碍。未后拉时预览 65% 力度，后拉时跟随选定力度。方向、力度、球杆和触球点仍由玩家决定。

2026-09-08 手感调整：鼠标返回时球杆同步下摆，越过击球线后 70 毫秒触球，避免再次重播完整下杆动作。触球提示给出力度、甜点击球或实际偏角，以及挑高/地滚球结果。轻微手抖不惩罚，球离杆后仍按原坡度和摩擦滚动，洞口判定不变。反馈是否符合个人操作习惯仍需玩家实玩判断。
...
- 地形与球物理：492 项检查，覆盖三个洞、八个方向、0.4～30 米／秒及 10/30/144 Hz 下的贴地过洞；包含快速内沿入洞、低空越洞不吸入、洞外擦过不吸入，以及原有地形、重力、摩擦和落杯检查。
- 高尔夫集成：138 项检查。包含真实场景碰撞下 12 米／秒滚动过洞，及快慢推同力度、轻微抖动容错、推球精度、满格行程、误触保护、跨击球线插值、实际物理短轨迹、Shift 精调、下杆跟手与所示力度一致；保留 R 重开、进洞结算、超过 12 杆继续本洞、存读档、鼠标与镜头检查。
- 取消进洞速度上限后，共 630 项检查通过（138 项高尔夫集成 + 492 项地形与球物理）。此前手感版本还通过了实际 OpenGL 窗口复跑；本次入洞修复使用无窗口物理与正式场景集成验证。
...
手感调整的界面截图脚本为 `tests/capture_golf_feel.gd`，使用 `--farm-test` 在真实图形窗口运行，输出到忽略目录 `tmp/golf-feel/`。已检查 1440×960 后拉/推球及 900×720 操作面板。自动检查能确认输入与物理结果一致，主观手感还需要玩家实际操作评估。
