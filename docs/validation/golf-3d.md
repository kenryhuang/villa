...
| 球洞 | 距离 | 特征 |
| --- | --- | --- |
| 草甸直道 | 32 米 | 缓丘接浅谷，越过丘顶后下坡攻果岭 |
| 沙丘弯道 | 40 米 | 沙坑旁隆起与侧肩，需控制落点和落地后的偏转 |
| 湖风长道 | 46 米 | 较高的长丘与侧坡，低处球路和越丘击球各有难度 |

三条球道均有超过 1.5 米的高差，丘体采用宽缓过渡；发球台和洞口附近保持平整，外围果岭保留轻微倾斜。沙坑下凹并带有平滑坡沿。

[草甸缓丘](images/golf_3d_meadow_hills.png) · [沙丘球道](images/golf_3d_dune_hills.png) · [长丘与侧坡](images/golf_3d_side_slope.png)

每洞标准杆 3，共 9 杆；每洞上限 12 杆。草地、果岭和沙坑使用与球物理一致的区域定义。球按重力飞行、落地反弹；滚动时根据实际地形三角面的坡度计算重力分量，上坡减速，下坡加速，横坡会向低处偏转。球在陡坡上减速到转折点后可以回滚。滚动阻力依次为果岭、球道、长草、沙坑递增，坡度和材质共同影响停止距离。
...
- 地形与球物理：180 项检查，覆盖三洞高差、平整发球台与洞口、多方向／速度／帧率进洞、内沿进洞、高速／空中越洞、擦洞、连续路径判定、上坡回滚、下坡加速、横坡偏转和四种材质阻力。
- 高尔夫集成：104 项检查，包含真实地形碰撞下的进洞，以及山丘网格高度、法线与球物理一致性；保留鼠标操作、镜头、计分、存档和界面中断检查。
- 相关回归：地形 64 项、南湖 52 项、正式场景 52 项。共 452 项；高尔夫在实际 OpenGL 窗口复跑不重复计数。12 张截图来自正式场景。
