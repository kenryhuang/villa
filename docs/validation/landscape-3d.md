...
正式入口仍为 `scenes/farm3d/main.tscn`。可探索地形由约 44 × 44 米扩展至 **160 × 160 米**，采用原有手绘草地贴图、橡树和暖色环境光。没有添加草丛杂草。

![地图全景](images/landscape_3d_overview.png)
...
`scripts/farm3d/terrain_profile.gd` 定义确定性的高度与区域，`landscape.gd` 生成 100 个 16 米地形分块和对应碰撞、河水、桥梁及橡树；`assets/terrain/landscape.gdshader` 和 `river.gdshader` 负责地面与水面。地形当前在 Godot 中生成，原角色、树木和作物继续使用 Blender 模型。
