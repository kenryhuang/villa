...
独立使用 `user://farm_3d_save.json`，写临时文件完成后替换。v3 保存原网格、作物进度、库存、时间、体力经验、水田、建筑施工、生产和金币，并加入市场行情、NPC 经济状态及实体市集位置。兼容 v1/v2；v1 一次性补充目录种子和材料，后续读取不重复补充。旧农田或建筑若占用默认市集位置，会选择其他空地。[市集操作与验证](market-3d.md)。
