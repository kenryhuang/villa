...
func run(assertions: TestAssert) -> void:
	...
	assertions.truthy(not bool(preview.get("regrowing", true)), "repeat crop harvest disables retained regrowth")
	assertions.equal(preview.get("post_cell_state", -1), GridCell.State.FARMLAND, "repeat crop returns to farmland")
	assertions.equal(preview.get("post_crop", "missing"), null, "repeat crop snapshot is cleared")
	var harvested := farming.harvest(regrow_cell, preview)
	assertions.truthy(not harvested.is_empty(), "repeat crop harvest commits")
	assertions.equal(regrow_cell.state, GridCell.State.FARMLAND, "repeat crop cell becomes empty farmland after harvest")
	assertions.truthy(regrow_cell.crop_instance == null, "repeat crop instance is removed after harvest")
