...
func _test_role_schedule_and_backpressure(assertions: TestAssert) -> void:
	...
	assertions.equal(failures.size(), 1, "dialogue replacement reports one explicit cancellation")
	assertions.equal(failures[0], {"agent_id": "farmer_ahe", "request_id": first_farmer_request_id, "error": "dialogue_replaced"}, "replacement cancellation identifies the old request")
