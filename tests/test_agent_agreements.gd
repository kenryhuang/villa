...
func run(assertions: TestAssert) -> void:
	...
	assertions.truthy(interactions.configure_player_assets(player_inventory, player_wallet), "agreement fixture configures authoritative Player assets")
	...
	assertions.truthy(agreements.validate_against_events(agreements.to_dict(), store.get_events_after(0)), "agreement snapshot matches its event history")
	var tampered_agreement := agreements.to_dict()
	tampered_agreement.agreements = (tampered_agreement.agreements as Array).duplicate(true)
	tampered_agreement.agreements[0] = (tampered_agreement.agreements[0] as Dictionary).duplicate(true)
	tampered_agreement.agreements[0].agreement = (tampered_agreement.agreements[0].agreement as Dictionary).duplicate(true)
	tampered_agreement.agreements[0].agreement.status = "cancelled"
	assertions.truthy(agreements.validate_dict(tampered_agreement), "structurally valid agreement tamper passes shape validation")
	assertions.truthy(not agreements.validate_against_events(tampered_agreement, store.get_events_after(0)), "agreement tamper is rejected by event history")
	...
	(player_terms.commitments as Array)[1] = {"participant_id": "player", "items": {"salt": 1}, "gold": 2}
	...
	assertions.equal(interactions.available_item("player", "salt"), player_salt_before - 1, "Player commitment is reserved while agreement is active")
	assertions.equal(interactions.available_gold("player"), player_gold_before - 2, "Player gold commitment is reserved while agreement is active")
	assertions.equal(agreements.record_action_outcome({"status": "completed", "agent_id": "farmer_ahe", "tool_name": "harvest", "action_id": "player-harvest", "agreement_id": str(player_agreement.agreement_id)}, 503).size(), 1, "Player agreement harvest milestone records")
	assertions.truthy(agreements.record_action_outcome({"status": "completed", "agent_id": "farmer_ahe", "tool_name": "sell", "action_id": "player-sell", "agreement_id": str(player_agreement.agreement_id)}, 504).size() >= 2, "Player agreement completes through linked milestones")
	assertions.equal(player_inventory.get_item_count("salt"), player_salt_before - 1, "Player commitment item is consumed at completion")
	assertions.equal(int(player_wallet.gold), player_gold_before + 48, "Player receives reward share after committed gold is consumed")
