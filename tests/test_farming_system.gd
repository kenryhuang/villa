...
func _test_exact_harvest_post_states(assertions: TestAssert) -> void:
	...
	assertions.equal(regrow_preview.get("regrowing", true), false, "repeat crop preview disables retained regrowth")
	assertions.equal(regrow_preview.get("post_crop", "missing"), null, "repeat crop preview removes the crop")
	assertions.equal(regrow_preview.get("post_lifecycle_state", "missing"), null, "repeat crop preview has no post lifecycle")
	assertions.equal(regrow_preview.get("post_cell_state", -1), GridCell.State.FARMLAND, "repeat crop preview returns empty farmland")
...
func _test_deterministic_harvest_transaction(assertions: TestAssert) -> void:
	...
	assertions.equal(first.get("post_lifecycle_state", "missing"), null, "preview clears the crop lifecycle")
	assertions.equal(first.get("post_cell_state", -1), GridCell.State.FARMLAND, "preview includes empty farmland post state")
	...
	var owned_token := farming.prepare_harvest(cell, first)
	assertions.truthy(owned_token is RefCounted, "harvest preparation returns unforgeable ownership")
	assertions.equal(farming.prepare_harvest(cell, first), null, "nested harvest preparation is rejected")
	...
	assertions.equal(cell.state, GridCell.State.FARMLAND, "successful repeat commit leaves empty farmland")
	assertions.truthy(cell.crop_instance == null, "successful repeat commit clears the crop")
	assertions.truthy(farming.get_crop_visual(cell) == null, "successful repeat commit removes the crop visual")
...
func _test_reentrant_harvest_observes_final_state(assertions: TestAssert) -> void:
	...
	assertions.equal(regrow_observer.observed_cell_state, GridCell.State.FARMLAND, "repeat listener observes cleared farmland")
	assertions.equal(regrow_observer.observed_visual_state, -1, "repeat listener observes the removed crop visual")
	assertions.truthy(regrow_observer.replanted != null, "repeat listener can replant the cleared farmland")
	assertions.equal(regrow_cell.crop_instance.crop_data.crop_id, "turnip", "replacement crop owns the cleared farmland")
	assertions.truthy(farming.get_crop_visual(regrow_cell) != null, "replacement crop has a visual")
