...
func _test_scene_toasts_center_and_hud(assertions: TestAssert, tree: SceneTree) -> void:
	...
	assertions.truthy(not ui.has_node("ToastStack"), "notification UI no longer owns a duplicate toast stack")
	assertions.truthy(not ui.has_method("advance_toasts"), "notification UI removes toast lifecycle APIs")
	...
	assertions.equal(hud.get_urgent_summary_count(), 0, "HUD removes the duplicate urgent summary outlet")
	assertions.truthy(not hud.has_node("UrgentSummaries"), "urgent records use the unified message stream")
