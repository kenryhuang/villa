...
func _test_task7_unversioned_inventory_public_load(
	...
	assertions.truthy(main.agent_runtime.validate_dict(no_crop_payload.agent_world), "repeated unversioned Agent snapshot remains valid")
	var repeated_migrated: Variant = manager._migrate_save_data(no_crop_payload)
	assertions.truthy(repeated_migrated is Dictionary, "repeated unversioned payload still migrates")
	assertions.truthy(repeated_migrated is Dictionary and manager._validate_save_data(repeated_migrated), "repeated migrated payload remains valid before apply")
