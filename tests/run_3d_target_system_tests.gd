...
func _run() -> void:
	...
	var size: Vector2 = Vector2(session.buildings._resolve_data("greenhouse").footprint)
	var bounds := Rect2(building_cell.world_position() - Vector2.ONE * .5, size)
	var center := bounds.get_center()
	for point in [Vector2(center.x, bounds.position.y - 1.2), Vector2(center.x, bounds.end.y + 1.2), Vector2(bounds.position.x - 1.2, center.y), Vector2(bounds.end.x + 1.2, center.y)]:
		player.position = Vector3(point.x, building_cell.terrain_height, point.y)
		_check(session.buildings.diagnose_placement("greenhouse", 26, 23).allowed, "Building can be reached from each footprint edge")
	player.position = Vector3(bounds.end.x + 2.59, building_cell.terrain_height, center.y)
	_check(session.buildings.diagnose_placement("greenhouse", 26, 23).allowed, "Building accepts positions within 2.6 metres of its edge")
	player.position.x = bounds.end.x + 2.61
	var rejected := session.apply_target(building_cell, "building", "greenhouse")
	_check(not rejected.ok and rejected.reason == "out_of_range" and "建筑边缘" in rejected.message, "Distant placement explains the building edge distance")
	player.position = Vector3(bounds.end.x + 2, building_cell.terrain_height, bounds.end.y + 2)
	_check(session.buildings.diagnose_placement("greenhouse", 26, 23).code == "out_of_range", "Diagonal reach uses actual distance rather than a square expansion")
	player.position = Vector3(center.x, building_cell.terrain_height, center.y)
	_check(session.apply_target(building_cell, "building", "greenhouse").reason == "player_in_footprint", "Player must remain outside the complete building footprint")
	player.position = Vector3(bounds.end.x + 1.2, building_cell.terrain_height + 5, center.y)
	_check(session.buildings.diagnose_placement("greenhouse", 26, 23).code == "out_of_range", "Vertical separation still prevents building from above a cliff")
	player.position.y = building_cell.terrain_height
	_check(not session._in_range(building_cell), "Regression fixture is outside the farming range of the footprint origin")
	_check(session.apply_target(building_cell, "farmland", "dry").reason == "out_of_range", "Farming retains its original single-cell reach")
	_check(session.buildings.enter_preview_mode("greenhouse") and session.buildings.update_preview_grid(26, 23), "Building preview agrees with placement when standing by its far edge")
	...
	for field in ["market", "npc_economy", "market_site", "golf", "agents", "living_world"]:
		legacy.erase(field)
