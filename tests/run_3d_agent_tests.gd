...
func _run() -> void:
	...
	check(env.characters.size() == 4 and env.map.has("market") and env.map.has("lake"), "Environment includes player, NPCs, real market and lake")
	...
	check(outcome.status == "completed", "Agent executor places rental order")
