...
func run() -> void:
	...
	var result := s.buildings.try_place_building("greenhouse",32,31)
	...
	var target := building.position+Vector3.UP*1.1
	...
	for shot in [["front",Vector3(7,5.7,9)],["rear",Vector3(-7,4.7,-8)]]:
		camera.position=target+shot[1];camera.look_at(target)
		await capture(shot[0])
	camera.position=target+Vector3(7,5.7,9);camera.look_at(target)
	interaction.hud.show();interaction.hud.open_windmill(building)
	await capture("panel")
	interaction.hud.close_panels();interaction.hud.hide()
	building.restore_construction(1,3)
	await capture("construction")
	print("GREENHOUSE CAPTURE: 4 views")
	quit(0)

func capture(suffix: String) -> void:
	for i in 10: await process_frame
	await RenderingServer.frame_post_draw
	root.get_texture().get_image().save_png("res://docs/validation/images/greenhouse_3d_"+suffix+".png")
