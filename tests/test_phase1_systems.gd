...
func _test_game_data(assertions: TestAssert) -> void:
	...
	assertions.equal(data.get_building("barn").cost.plank, 8, "building processed-material cost is available")
...
func _test_inventory(assertions: TestAssert) -> void:
	...
	assertions.equal(inventory.slots[0].quantity, 20, "remove compacts the remaining partial stack")
	assertions.truthy(inventory.slots[1].is_empty(), "remove clears duplicate underfilled stacks")
	assertions.truthy(not inventory.remove_item("wood", 21), "cannot remove unavailable quantity")
	assertions.truthy(inventory.set_quick_slot(0, 0), "quick slot maps inventory slot")
...
func _test_inventory_ui(assertions: TestAssert) -> void:
	...
	assertions.truthy(ui.get_viewport() == null, "script-only inventory UI fixture stays off-tree")
	ui._ready()
	assertions.truthy(ui.inventory_grid != null, "off-tree ready creates the inventory grid")
	assertions.truthy(ui.quick_bar != null, "off-tree ready creates the quick bar")
