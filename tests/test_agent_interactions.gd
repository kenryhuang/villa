...
func run(assertions: TestAssert) -> void:
	...
	assertions.truthy(interactions.validate_against_events(interactions.to_dict(), store.get_events_after(0)), "trade snapshot matches its event history")
	var tampered_trade := interactions.to_dict()
	tampered_trade.offers = (tampered_trade.offers as Array).duplicate(true)
	tampered_trade.offers[0] = (tampered_trade.offers[0] as Dictionary).duplicate(true)
	tampered_trade.offers[0].offer = (tampered_trade.offers[0].offer as Dictionary).duplicate(true)
	tampered_trade.offers[0].offer.status = "cancelled"
	assertions.truthy(interactions.validate_dict(tampered_trade), "structurally valid trade tamper passes shape validation")
	assertions.truthy(not interactions.validate_against_events(tampered_trade, store.get_events_after(0)), "trade tamper is rejected by event history")
	...
	assertions.truthy(interactions.reserve_assets("farmer_ahe", "save-lock", {"items": {}, "gold": 1}), "save fixture creates an external resource lock")
	var saved := interactions.to_dict()
	var json_saved: Dictionary = JSON.parse_string(JSON.stringify(saved))
	var restored := InteractionScript.new()
	assertions.truthy(restored.configure(economy, store, projector, Callable(), market), "restored interaction system configures")
	assertions.truthy(restored.configure_player_assets(player_inventory, player_wallet), "restored interaction system reconnects Player assets")
	assertions.truthy(restored.from_dict(json_saved), "pending offers, locks and price pressure restore after JSON round trip")
	var restored_snapshot := restored.to_dict()
	assertions.equal(restored.to_dict(), restored_snapshot, "interaction persistence is deterministic")
	assertions.equal(restored.available_gold("farmer_ahe"), interactions.available_gold("farmer_ahe"), "restored external lock still limits spendable gold")
	market.free()
	economy.free()
