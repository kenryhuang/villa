...
这一组原创模型使用 Blender 程序化建模，供第三人称画面和比例评估。角色和田地采用简化卡通形体与纯色粗糙材质；场景树木使用新建的手绘橡树模型。
...
| `farm_environment.glb` | 九块田、幼苗、麦穗、小路、草、石头、围栏；不包含树木 |
...
项目默认启动场景为 `scenes/preview/farm_3d_preview.tscn`，在 Godot 中按 F5 即可进入 3D 农庄，也可以运行 `./tools/preview_3d.ps1`。目前用于模型与操控评估，尚未接入种植/收获交互。原游戏保留在 `scenes/main.tscn`，可单独打开后按 F6 运行；单树观察场景为 `scenes/preview/tree_3d_preview.tscn`。
