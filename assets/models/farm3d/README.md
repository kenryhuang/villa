...
项目默认启动场景为 `scenes/farm3d/main.tscn`，在 Godot 中按 F5 即可进入 3D 农庄，也可以运行 `./tools/run_farm3d.ps1`。从底部两级菜单选择农田、种子或建筑，靠近后左键操作鼠标指向的格子；Esc 收起目标，空手点击作物可浇水、收获或清理，I 打开背包。原游戏保留在 `scenes/main.tscn`，可单独打开后按 F6 运行；独立橡树观察场景为 `scenes/preview/tree_3d_preview.tscn`。
