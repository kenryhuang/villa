...
const TOOL_PARAMETERS: Readonly<Record<string, JsonSchema>> = {
  ...
  start_leisure: objectSchema({activity: {type: "string", enum: ["visit", "companionship", "date", "rest", "sleep", "eat", "drink"]}, partner_id: ITEM_ID_SCHEMA}, ["activity", "partner_id"]),
  resolve_relationship_dialogue: objectSchema({player_quote:{type:"string",minLength:1,maxLength:8000},decision:{type:"string",enum:["confirm","decline","end"]},note:{type:"string",minLength:1,maxLength:500}},["player_quote","decision","note"]),
  propose_relationship: objectSchema({target_actor_id:ITEM_ID_SCHEMA,note:{type:"string",minLength:1,maxLength:500}},["target_actor_id","note"]),
  respond_relationship: objectSchema({proposal_id:ITEM_ID_SCHEMA,version:{type:"integer",minimum:1},accept:{type:"boolean"}},["proposal_id","version","accept"]),
  end_relationship: objectSchema({target_actor_id:ITEM_ID_SCHEMA,reason:{type:"string",minLength:1,maxLength:500}},["target_actor_id","reason"]),
  express_support: objectSchema({target_actor_id:ITEM_ID_SCHEMA,text:{type:"string",minLength:1,maxLength:500}},["target_actor_id","text"]),
...
export function validToolArguments(name: string, value: unknown): boolean {
  ...
  switch (name) {
    ...
    case "start_leisure": return hasExactKeys(value, ["activity", "partner_id"]) && isOneOf(value.activity, ["visit", "companionship", "date", "rest", "sleep", "eat", "drink"]) && isBoundedId(value.partner_id);
    case "resolve_relationship_dialogue": return hasExactKeys(value,["player_quote","decision","note"]) && isText(value.player_quote,8000) && ["confirm","decline","end"].includes(String(value.decision)) && isText(value.note,500);
    case "propose_relationship": return hasExactKeys(value,["target_actor_id","note"]) && isBoundedId(value.target_actor_id) && isText(value.note,500);
    case "respond_relationship": return hasExactKeys(value,["proposal_id","version","accept"]) && isBoundedId(value.proposal_id) && isIntegerInRange(value.version,1,1000000) && typeof value.accept==="boolean";
    case "end_relationship": return hasExactKeys(value,["target_actor_id","reason"]) && isBoundedId(value.target_actor_id) && isText(value.reason,500);
    case "express_support": return hasExactKeys(value,["target_actor_id","text"]) && isBoundedId(value.target_actor_id) && isText(value.text,500);
...
export function toolDescription(name: string, physical3d = false): Record<string, unknown> {
  ...
  const farmingDescriptions: Record<string, string> = {
    ...
    start_leisure: "Meet personal needs independently of occupation. Query self/needs first. rest (60 minutes), sleep (180), eat (15, consumes one real food item) require your own actor ID. drink (10) requires village_inn. visit/companionship (60) require another available person; walk there and spend real time together to improve affection and social/companionship needs. date (60) allows two single adults to voluntarily get to know each other, or existing mutual partners. No prior formal relationship or affection threshold is required for dating. Respect willingness expressed in dialogue; stop if one becomes unavailable or declines. Scheduling returns in_progress. Same-batch actions wait until actual completion; do not resubmit while busy. Completion only; no invented consent.",
    resolve_relationship_dialogue: "Dialogue only: interpret the current player message and record mutual confirmation, refusal or breakup. player_quote must copy the ENTIRE current player input exactly; old dialogue, memory and background loops cannot authorize this. confirm ONLY if the player explicitly asks to become romantic partners or accepts your existing proposal AND you independently want to accept. An invitation to date, a compliment, roleplay example, hypothetical, negation or uncertain reply is NOT confirmation. Ask naturally if ambiguous. decline records either refusal; end records the player explicitly ending your current relationship. Query actors/relationship with id=player first; live status overrides memory. If status=dating, acknowledge the existing relationship without confirming again. An empty proposals list does not mean single. Include a brief spoken reply along with a new relationship action. No dedicated UI exists. Single adults may date before this formal confirmation.",
    propose_relationship: "Express your own wish to date another adult. Query actors/relationship with their id first: both affection scores >=60 and at least 3 personal shared experiences are required. High affection permits a proposal, never automatic consent. This creates a pending proposal; the recipient independently responds. Player replies through ordinary dialogue; resolve_relationship_dialogue records explicit consent from the current player input. Consider your social_profile, preferences and romance interest. Respect rejection and cooldown; never purchase consent.",
    respond_relationship: "Respond only to a relationship proposal addressed to you, at its current version from actors/relationship_proposals. Independently choose accept=true or false based on your personality and wishes. Never act as the player. Dating begins only when both have agreed; refusal does not reduce affection.",
    end_relationship: "End your own existing romantic relationship with target_actor_id. Either partner may leave; no permission from the other is required. Preserve shared history and respect the subsequent cooldown.",
    express_support: "Actually say the supplied empathetic text to a nearby person (within 4 world units). Choose sincere comfort relevant to their situation, without claiming to know private thoughts. Delivered support can increase recipient affection and relieve companionship need; daily caps prevent repeated lines farming points. Cannot comfort remotely or claim a relationship through speech.",
    ...
    adopt_short_term_goal: "Persist a specific accepted plan for the next background Agent Loop. During dialogue, record what you explicitly agree to do, the destination/person, constraints and completion criteria; use turn.dialogue_event_id in source_event_ids and review_in_minutes=1. This records intention only and does not execute movement or work. Do not create goals for greetings, refusals, hypothetical suggestions or duplicate active goals. Discover live facts before later submitting concrete action tools.",
    suggest_behavior: "Record an optional general behavioral preference, not a concrete action promise. For an agreed trip, work or other actionable plan use adopt_short_term_goal so the next background Loop receives a persistent task. text must be at most 500 characters; aim for under 300. Do not include a wait command in the same response.",
