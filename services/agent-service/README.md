...
P6 adds player-confirmed `propose_delivery`, versioned `cancel_delivery`, and `revise_project`. Dialogue keeps accepted task terms and the current suspended project in context; Godot validates timing, cargo escrow, physical arrival and resumption. P7 registers `village_public` through `data/agents/public_coordinator.json`, with only `public_food_plan` and `public_wait`. It receives authorized aggregate food indicators, public price references, budget and existing measures. It cannot inherit private commands, inspect private facts or switch into a private role. Both coordinator and NPC calls share the original Provider concurrency limit and session memory store. See [P6 validation](../../docs/validation/living-world/P6.md) and [P7 validation](../../docs/validation/living-world/P7.md).
...
Dialogue requests set `enable_thinking: false`. Autonomous `event`, `schedule`, and `catch_up` requests explicitly enable thinking, including voluntary negotiations and population plans marked `concise`. Concise planning limits response length; it does not disable reasoning. Only the bounded correction/timeout retries described below disable thinking for recovery.
...
Command argument errors, contradictory `wait` batches, excessive command counts and incomplete/truncated output also share that single correction attempt. The rejected batch executes no commands; the model must regenerate its intended batch from existing context, with thinking disabled and no new reads. Feedback identifies field limits (including 500-character behavior text/project goals and the public procurement price cap of 200). Invalid JSON stays intact in the raw trace, and rejected outputs carry `validation_errors`. A second invalid command response remains an error; permissions, dialogue limits and cancellation still apply. See `tests/provider_recovery.test.ts` for the September 9 trace regressions.
