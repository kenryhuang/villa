...
test("accepts exact Provider tool arguments for every contract shape", () => {
  const validCases: Array<[string, Record<string, unknown>]> = [
    ...
    ["counter_cooperation", {agreement_id: "agreement-1", revised_terms: {commitments: [{participant_id: "farmer_ahe", items: {carrot_seed: 1}, gold: 0}, {participant_id: "lao_li", items: {salt: 1}, gold: 0}], reward_split: {farmer_ahe: 1, lao_li: 1}, deadline_minutes: 180}, note: "延长期限"}],
    ["accept_cooperation", {agreement_id: "agreement-1", terms_version: 2}],
    ["reject_cooperation", {agreement_id: "agreement-1", reason_code: "not_ready"}],
    ["commit_contribution", {agreement_id: "agreement-1", contribution_id: "deliver_material"}],
    ["cancel_cooperation", {agreement_id: "agreement-1", reason_code: "plans_changed"}],
